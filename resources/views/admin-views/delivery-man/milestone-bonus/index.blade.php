...
@section('content')
    <div class="content container-fluid">
        <div class="page-header">
            ...
            <div class="mt-2">
                <a href="{{ route('admin.users.delivery-man.milestone-bonus.awards') }}" class="btn btn-sm btn--primary">{{ translate('messages.dm_bonus_awards') }}</a>
            </div>
        </div>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form action="{{ route('admin.users.delivery-man.milestone-bonus.store') }}" method="post" class="row align-items-end">
                    @csrf
                        ...
                        <tbody>
                            @forelse($rows as $m)
                                <tr>
                                    <td>{{ $m->slot }}</td>
                                    <td colspan="3">
                                        <form action="{{ route('admin.users.delivery-man.milestone-bonus.update', $m->id) }}" method="post" class="form-inline flex-wrap align-items-center">
                                            @csrf
                                    ...
                                    <td>
                                        <form action="{{ route('admin.users.delivery-man.milestone-bonus.delete', $m->id) }}" method="post" class="d-inline" onsubmit="return confirm('{{ translate('messages.are_you_sure') }}');">
                                            @csrf
                                            @method('DELETE')
