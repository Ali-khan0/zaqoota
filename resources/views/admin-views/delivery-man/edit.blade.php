...
@section('content')
    <div class="content container-fluid">
        ...
        @if(!empty($regFeeSettings['enabled']) || ($registrationFee ?? null))
            <div class="card mb-3">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        <span class="card-header-icon"><i class="tio-money"></i></span>
                        {{ translate('messages.dm_edit_registration_fee_section') }}
                    </h5>
                </div>
                <div class="card-body">
                    @if(!empty($regFeeSettings['enabled']))
                        <div class="row">
                            <div class="col-md-4 mb-2">
                                <span class="d-block text-muted small">{{ translate('messages.dm_registration_fee_total') }}</span>
                                <span class="font-weight-bold">{{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($registrationFee->total_fee ?? $regFeeSettings['total_fee'], 2) }}</span>
                                @unless($registrationFee ?? null)
                                    <span class="badge badge-soft-info ml-1">{{ translate('messages.default') }}</span>
                                @endunless
                            </div>
                            <div class="col-md-4 mb-2">
                                <span class="d-block text-muted small">{{ translate('messages.dm_approve_initial_amount') }} ({{ translate('messages.recorded') }})</span>
                                @if($registrationFee && $registrationFee->manual_confirmed)
                                    <span class="font-weight-bold text-success">{{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($registrationFee->manual_paid_amount, 2) }}</span>
                                    <span class="text-muted small d-block">{{ \App\CentralLogics\Helpers::time_date_format($registrationFee->manual_confirmed_at) }}</span>
                                @else
                                    <span class="font-weight-bold text-muted">—</span>
                                    <span class="d-block small text-muted">{{ translate('messages.dm_edit_initial_not_recorded') }}</span>
                                @endif
                            </div>
                            <div class="col-md-4 mb-2">
                                <span class="d-block text-muted small">{{ translate('messages.remaining_due') }}</span>
                                <span class="font-weight-bold">{{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($registrationFee->wallet_remaining_due ?? $regFeeSettings['total_fee'], 2) }}</span>
                                @if($registrationFee && $registrationFee->completed_at)
                                    <span class="badge badge-soft-success ml-1">{{ translate('messages.completed') }}</span>
                                @endif
                            </div>
                        </div>
                        <p class="mb-2 small text-muted">{{ translate('messages.dm_edit_initial_expected_hint') }} {{ \App\CentralLogics\Helpers::currency_symbol() }}{{ number_format($regFeeSettings['manual_first_part'], 2) }}</p>
                        <a class="btn btn-sm btn--secondary" href="{{ route('admin.users.delivery-man.registration-fee') }}">{{ translate('messages.dm_registration_fee_settings') }}</a>
                    @else
                        <p class="mb-0 text-muted">{{ translate('messages.dm_reg_fee_disabled_edit_hint') }}</p>
                    @endif
                </div>
            </div>
        @endif
                                ...
                                <div class="col-sm-6 col-lg-12">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="identity_type" class="form-control  js-select2-custom">
                                            ...
                                            <option value="nid" {{$deliveryMan['identity_type']=='nid'?'selected':''}}>{{translate('messages.CNIC')}}
                                            </option>
                                        </select>
                                    </div>
                                </div>
