<div class="h-100vh-150px">
    ...
    <div class="card mb-3">
        <div class="card-header">
            <h6 class="mb-0 font-medium">{{ translate('messages.customer_info') }}</h6>
        </div>
        <div class="card-body">
            <div class="key-val-list d-flex flex-column mb-2 gap-2" style="--min-width: 60px">
                <div class="key-val-list-item d-flex gap-3">
                    <span>{{ translate('Name') }}:</span>
                    <span>{{ $withdraw->user->f_name.' '.$withdraw->user->l_name }}</span>
                </div>
            </div>
            <div class="key-val-list d-flex flex-column mb-2 gap-2" style="--min-width: 60px">
                <div class="key-val-list-item d-flex gap-3">
                    <span>{{ translate('Email') }}:</span>
                    <span class="text-title">{{ $withdraw->user->email }}</span>
                </div>
            </div>
            <div class="key-val-list d-flex flex-column mb-2 gap-2" style="--min-width: 60px">
                <div class="key-val-list-item d-flex gap-3">
                    <span>{{ translate('Phone') }}:</span>
                    <span class="text-title">{{ $withdraw->user->phone }}</span>
                </div>
            </div>
            ...
            <div class="rounded bg-light p-3 mt-3">
                <div class="key-val-list-item d-flex gap-3 align-items-center lh-1">
                    ...
                    <span class="font-semibold text-primary fs-16">{{ \App\CentralLogics\Helpers::format_currency($withdraw->user->wallet_balance ?? 0) }}</span>
                </div>
            </div>
        </div>
    </div>
