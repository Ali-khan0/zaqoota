...
@section('title', translate('Customer Withdraw Request'))
    ...
    <div class="content container-fluid">
        <div class="page-header">
            <h1 class="page-header-title mr-3 mb-md-0">
                ...
                <span>
                    {{ translate('messages.customer_withdraw_transaction') }}
                </span>
            </h1>
        </div>
                ...
                <div class="search--button-wrapper justify-content-end">
                    <form class="search-form theme-style">
                        <div class="input-group input--group">
                            <input id="datatableSearch" name="search" type="search" value="{{ request()?->search ?? null }}" class="form-control h--40px" placeholder="{{ translate('messages.search_here') }}" aria-label="{{ translate('messages.search_here') }}">
                    ...
                    <div class="max-sm-flex-1">
                        <select name="withdraw_status_filter" class="custom-select h--40px py-0 status-filter theme-style">
                            <option value="all" {{ session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'all' ? 'selected' : '' }}>
                                {{ translate('messages.all') }}
                            </option>
                            <option value="approved" {{ session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'approved' ? 'selected' : '' }}>
                                {{ translate('messages.approved') }}
                            </option>
                            <option value="denied" {{ session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'denied' ? 'selected' : '' }}>
                                {{ translate('messages.denied') }}
                            </option>
                            <option value="pending" {{ session()->has('withdraw_status_filter') && session('withdraw_status_filter') == 'pending' ? 'selected' : '' }}>
                    ...
                    <div class="hs-unfold mr-2">
                        ...
                        <div id="usersExportDropdown" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-sm-right">
                            ...
                            <a id="export-excel" class="dropdown-item" href="{{ route('admin.transactions.customer.withdraw_export', ['type'=>'excel', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2" src="{{ asset('public/assets/admin') }}/svg/components/excel.svg" alt="">
                                {{ translate('messages.excel') }}
                            </a>
                            <a id="export-csv" class="dropdown-item" href="{{ route('admin.transactions.customer.withdraw_export', ['type'=>'csv', request()->getQueryString()]) }}">
                    ...
                    <table id="datatable" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                        ...
                        @foreach($withdraw_req as $k => $wr)
                            <tr>
                                ...
                                <td>
                                    @if($wr->user)
                                        {{ $wr->user->f_name }} {{ $wr->user->l_name }} ({{ $wr->user->phone }})
                                    @else
                                        {{ translate('messages.customer_deleted') }}
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if($wr->user)
                                        <a href="#" data-id="{{ $wr->id }}" class="btn action-btn btn--warning btn-outline-warning withdraw-info-show">
...
@push('script_2')
    <script>
        ...
        $(document).on('click', '.show-approve-view', function () {
            let id = $(this).data('id');
            let url = "{{ route('admin.transactions.customer.withdraw_status', ['data_id']) }}";
            ...
            <form class="withdraw_status_form" action="${url}" method="POST">
                ...
                <div class="mt-5">
                    ...
                    <textarea required name="note" class="form-control" rows="6" maxlength="200" placeholder="{{ translate('Type_a_note_about_request_approval') }}"></textarea>
        ...
        $(document).on('click', '.show-deny-view', function () {
            ...
            <form class="withdraw_status_form" action="${url}" method="POST">
                ...
                <div class="mt-5">
                    ...
                    <textarea required name="note" class="form-control" rows="6" maxlength="200" placeholder="{{ translate('Type_a_note_about_request_denial') }}"></textarea>
        ...
        $('.status-filter').on('change', function () {
            ...
            $.post({
                url: '{{ route('admin.transactions.customer.status-filter') }}',
        ...
        function fetch_data(id) {
            $.ajax({
                url: "{{ route('admin.transactions.customer.getWithdrawDetails') }}" + '?withdraw_id=' + id,
                type: "get",
                beforeSend: function () {
                    $('#data-view').empty();
                    $('#loading').show();
