    ...
    <div class="card">
        <div class="card-header border-0">
            <form class="w-100"><div class="row g-2">
                <div class="col-md-8"><input type="search" name="search" value="{{ $search }}" class="form-control" placeholder="{{ translate('messages.Search rider name, phone or email') }}"></div>
                <div class="col-md-2"><select name="work_mode" class="form-control"><option value="">{{ translate('messages.All Work Modes') }}</option><option value="delivery" @selected($mode === 'delivery')>{{ translate('messages.Delivery Mode') }}</option><option value="ride" @selected($mode === 'ride')>{{ translate('messages.Ride Mode') }}</option></select></div>
        ...
        <div class="table-responsive"><table class="table table-hover table-borderless table-thead-bordered table-align-middle card-table">
            ...
            <tbody>@forelse($riders as $rider)<tr>
                <td><strong>{{ $rider->full_name }}</strong><br><small class="text-muted">{{ $rider->phone }}{{ $rider->email ? ' · '.$rider->email : '' }}</small></td>
        ...
        @if($riders->hasPages())<div class="card-footer">{{ $riders->links() }}</div>@endif
    </div>
</div>
@endsection
