...
@section('title', translate('messages.Register Ride Vehicle'))

@section('content')
<div class="content container-fluid">
    <div class="page-header"><h1 class="page-header-title"><span class="page-header-icon"><i class="tio-car"></i></span>{{ translate('messages.Register Ride Vehicle') }}</h1></div>
    ...
    <form action="{{ route('admin.ride-hailing.vehicles.store') }}" method="post">@csrf
        <div class="card"><div class="card-header"><h5 class="card-title">{{ translate('messages.Rider and Vehicle Details') }}</h5></div><div class="card-body"><div class="row g-3">
            <div class="col-12"><label class="input-label">{{ translate('messages.Rider') }}</label><select id="delivery_man_id" name="delivery_man_id" class="form-control" required></select><small class="text-muted">{{ translate('messages.Only approved riders with fewer than two ride vehicles appear.') }}</small></div>
        ...
        <div class="btn--container justify-content-end mt-4"><a href="{{ route('admin.ride-hailing.vehicles.index') }}" class="btn btn--reset">{{ translate('messages.Cancel') }}</a><button class="btn btn--primary">{{ translate('messages.Register Vehicle') }}</button></div>
    </form>
</div>
@endsection
