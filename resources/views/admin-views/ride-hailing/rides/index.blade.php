        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.ride-hailing.rides.index') }}">
                ...
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-sm-6 col-lg-2"><label class="input-label">{{ translate('messages.Payment') }}</label><select name="payment_status" class="form-control"><option value="all">{{ translate('messages.All') }}</option>@foreach(['due', 'due_next_ride', 'unpaid', 'pending', 'partially_paid', 'paid', 'recovered', 'not_required'] as $payment)<option value="{{ $payment }}" @selected($paymentStatus === $payment)>{{ ucfirst(str_replace('_', ' ', $payment)) }}</option>@endforeach</select></div>
            ...
            <table class="table table-hover table-borderless table-thead-bordered table-align-middle card-table">
                ...
                @forelse($rides as $ride)
                    ...
                    <tr>
                        ...
                        <td><span class="badge badge-soft-{{ $meta[1] }}"><i class="{{ $meta[2] }} mr-1"></i>{{ translate('messages.'.$meta[0]) }}</span>@if($ride->status === 'captain_arriving')<br><small class="text-primary">{{ translate('messages.The Captain is coming to pickup.') }}</small>@elseif($ride->status === 'cancelled')<br><small class="text-danger">{{ translate('messages.Cancelled by') }} {{ $ride->cancelled_by === 'customer' ? translate('messages.Passenger') : ($ride->cancelled_by === 'admin' ? translate('messages.Zaqoota Admin') : translate('messages.Captain')) }}</small>@endif</td>
