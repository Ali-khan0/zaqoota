                    ...
                    <div class="card">
                        <form action="{{env('APP_MODE')!='demo'?route('admin.business-settings.third-party.payment-method-update'):'javascript:'}}" method="POST"
                            ...
                            <div class="card-body">
                                ...
                                @if($payment['key_name'] == 'assan_pay')
                                    @php($assanPayCallback = env('APP_ENV')=='demo' ? '' : route('assan-pay.callback', absolute: true))
                                    <div class="form-floating mb-2" >
                                        <label for="AssanPay_Callback_Url" class="form-label">{{translate('Callback Url')}} (for AssanPay dashboard)</label>
                                        <input id="AssanPay_Callback_Url" type="text"
                                               class="form-control"
                                               placeholder="{{translate('Callback Url')}}"
                                               readonly
                                               value="{{ $assanPayCallback }}" {{$is_published}}>
                                    </div>
