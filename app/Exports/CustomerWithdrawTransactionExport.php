...
class CustomerWithdrawTransactionExport implements FromView, ShouldAutoSize, WithStyles, WithColumnWidths, WithHeadings, WithEvents
{
    use Exportable;
    ...
    {
        return view('file-exports.customer-withdraw-transaction', [
            'data' => $this->data,
        ]);
    }
    ...
    {
        $count = $this->data['withdraw_requests']->count();
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) use ($count) {
                ...
                $event->sheet->getStyle('A3:G'.($count + 3))
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER)
                    ->setVertical(Alignment::VERTICAL_CENTER);
