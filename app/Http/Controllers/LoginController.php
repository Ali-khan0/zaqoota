...
class LoginController extends Controller
{
    ...
    public function login($login_url)
    {
        ...
        // Check if GD extension is available for CAPTCHA
        ...
        if (extension_loaded('gd') && function_exists('imagecreatetruecolor')) {
            try {
                ...
                // Create a placeholder captcha object
                $custome_recaptcha = $this->createPlaceholderCaptcha();
                Session::put('six_captcha', '1234');
            }
        } else {
            // GD extension not available, create a placeholder captcha object
            Log::warning('GD extension not loaded. CAPTCHA disabled.');
            $custome_recaptcha = $this->createPlaceholderCaptcha();
            Session::put('six_captcha', '1234');
        }
    ...
    public function reloadCaptcha()
    {
        ...
        if (extension_loaded('gd') && function_exists('imagecreatetruecolor')) {
            try {
                ...
                Log::error('CAPTCHA reload failed: ' . $e->getMessage());
                $custome_recaptcha = $this->createPlaceholderCaptcha();
                Session::put('six_captcha', '1234');
                return response()->json([
                    'view' => view('auth.custom-captcha', compact('custome_recaptcha'))->render()
    ...
    /**
     * Create a placeholder CAPTCHA object when GD extension is not available
     */
    private function createPlaceholderCaptcha()
    {
        // Create a visible placeholder image as data URI (120x40 gray box with text)
        // This is a simple SVG converted to data URI
        $placeholderImage = 'data:image/svg+xml;base64,' . base64_encode(
            '<svg width="120" height="40" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="60" y="25" font-family="Arial" font-size="16" fill="#666" text-anchor="middle">1234</text>
            </svg>'
        );
        
        // Create a mock object that has the inline() method
        return new class($placeholderImage) {
            private $image;
