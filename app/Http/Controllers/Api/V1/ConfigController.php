...
if (addon_published_status('TaxModule')) {
    $systemTax = \Modules\TaxModule\Entities\SystemTaxSetup::where('is_active', 1)
        ->where('is_default', 1)
        ->first();
}
        ...
        return response()->json([
            ...
            'ride_realtime' => [
                'enabled' => (bool) (isset($settings['websocket_status']) ? $settings['websocket_status'] : 0),
                'driver' => config('broadcasting.default'),
                'key' => config('broadcasting.connections.'.config('broadcasting.default').'.key'),
                'host' => config('broadcasting.connections.'.config('broadcasting.default').'.options.host') ?: ($settings['websocket_url'] ?? ''),
                'port' => (int) (config('broadcasting.connections.'.config('broadcasting.default').'.options.port') ?: ($settings['websocket_port'] ?? 6001)),
                'scheme' => config('broadcasting.connections.'.config('broadcasting.default').'.options.scheme', 'https'),
                'customer_auth_endpoint' => url('/api/v1/ride-hailing/customer/broadcasting/auth'),
                'captain_auth_endpoint' => url('/api/v1/delivery-man/ride-broadcasting/auth'),
...
//                'meta_data_section' => $metaDataSection,
    ...
    private function getDefaultPaymentMethods()
    {
        ...
        $methods = Setting::where('is_active', 1)->whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'assan_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago'])->get();
