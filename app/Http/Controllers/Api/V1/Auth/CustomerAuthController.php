...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if ($validator->fails()) {
                throw new \Exception(json_encode(Helpers::error_processor($validator)), 403);
            }
             ...
             * ======================= */
            $ref_by = null;
            $refNotificationData = null;
            $refUser = null;
    
            if ($request->ref_code) {
    
                $ref_status = BusinessSetting::where('key', 'ref_earning_status')->value('value');
                if ($ref_status != '1') {
                    throw new \Exception(translate('messages.referer_disable'), 403);
                }
    
                $refUser = User::where('ref_code', $request->ref_code)->where('status', 1)->first();
                if (!$refUser) {
                    throw new \Exception(translate('messages.referer_code_not_found'), 405);
                }
    
                if (WalletTransaction::where('reference', $request->phone)->exists()) {
                    throw new \Exception(translate('Referrer code already used'), 403);
                }
            ...
            $firebase_otp_verification = BusinessSetting::where('key', 'firebase_otp_verification')->value('value') ?? 0;
             ...
             * 6. PHONE OTP
             * ======================= */
            if (($login_settings['phone_verification_status'] ?? 0) == 1) {
                ...
                if (!$firebase_otp_verification) {
                    ...
                    $otp_interval = 60;
                    $verification = DB::table('phone_verifications')->where('phone', $request->phone)->first();
    
                    if ($verification && Carbon::parse($verification->updated_at)->diffInSeconds() < $otp_interval) {
                        throw new \Exception(translate('messages.please_try_again_after_'), 405);
                    }
                    ...
                    if (env('APP_MODE') !== 'test' && $response !== 'success') {
                        throw new \Exception(translate('messages.failed_to_send_sms'), 405);
                    }
                }
            }
             ...
             * 7. EMAIL OTP
             * ======================= */
            elseif (($login_settings['email_verification_status'] ?? 0) == 1) {
             ...
             * ======================= */
            if ($refUser && $refNotificationData &&
                Helpers::getNotificationStatusData('customer', 'customer_new_referral_join', 'push_notification_status') &&
                $refUser->cm_firebase_token
            ) {
                Helpers::send_push_notif_to_device($refUser->cm_firebase_token, $refNotificationData);
                DB::table('user_notifications')->insert([
                    'data' => json_encode($refNotificationData),
                    'user_id' => $refUser->id,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            ...
            return response()->json([
                'errors' => [
                    ['code' => 'register', 'message' => $e->getMessage()]
                ]
            ], $e->getCode() ?: 403);
        }
    }
