        ...
        if (isset($customer)) {
            ...
            elseif(config('mail.status')){
                try {
                    ...
                    // Debug logging
                    info('Forget Password Mail Check:');
                    info('config(mail.status): ' . (config('mail.status') ? 'true' : 'false'));
                    info('forget_password_mail_status_user: ' . ($mail_status ?? 'null'));
                    info('Customer email: ' . ($customer['email'] ?? 'null'));
                    
                    if ($mail_status == '1' && !empty($customer['email'])) {
                        try {
                            Mail::to($customer['email'])->send(new UserPasswordResetMail($token, $customer['f_name']));
                            $mailResponse = 'success';
                            info('Forget password mail sent successfully to: ' . $customer['email']);
                        } catch (\Exception $mailEx) {
                            // SMTP connection or other mail errors
                            info('Forget Password Mail Exception: ' . $mailEx->getMessage());
                            info('Forget Password Mail Exception Class: ' . get_class($mailEx));
                            info('Forget Password Mail Exception Trace: ' . $mailEx->getTraceAsString());
                            $mailResponse = null;
                        }
                    } else {
                        // Log why mail wasn't sent
                        $reasons = [];
                        if ($mail_status != '1') {
                            $reasons[] = 'Forget password mail is not enabled in email format settings';
                            info('Forget password mail not sent: forget_password_mail_status_user is not enabled');
                        }
                        if (empty($customer['email'])) {
                            $reasons[] = 'Customer email is empty';
                            info('Forget password mail not sent: Customer email is empty');
                        }
                        info('Forget password mail not sent. Reasons: ' . implode(', ', $reasons));
                    }
                } catch (\Throwable $th) {
                    $mailResponse = null;
                    info('Forget Password Exception: ' . $th->getMessage());
                    info('Forget Password Exception Trace: ' . $th->getTraceAsString());
                }

                if ($mailResponse == 'success') {
                    return response()->json(['message' => translate('messages.Otp_Successfully_Sent_To_Your_Mail')], 200);
                } else {
                    $errorMessage = translate('messages.failed_to_send_mail');
                    
                    // Provide more specific error message
                    $mail_status = Helpers::get_mail_status('forget_password_mail_status_user');
                    if ($mail_status != '1') {
                        $errorMessage = 'Forget password email is disabled. Please enable it in email format settings.';
                    } elseif (empty($customer['email'])) {
                        $errorMessage = 'Customer email address is not available.';
                    } else {
                        $errorMessage = 'Failed to send mail. Please check your SMTP configuration and server logs for details.';
                    }
                    
                    return response()->json([
                        'errors' => [
                            ['code' => 'otp', 'message' => $errorMessage]
                    ]], 403);
                }
            }
            else
