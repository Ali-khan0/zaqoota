...
class WalletController extends Controller
{
    ...
    public function customer_withdraw_methods()
    {
        $methods = WithdrawalMethod::ofStatus(1)->get();
        return response()->json($methods ?? [], 200);
    }

    public function customer_withdraw_list(Request $request)
    {
        $user = $request->user();
        ...
        return response()->json([
            'total_size' => $withdraws->total(),
            'limit' => (int) $withdraws->perPage(),
            'offset' => (int) $withdraws->currentPage(),
            'data' => $withdraws->items(),
        ], 200);
    }

    public function customer_request_withdraw(Request $request)
    {
        $hasActiveMethods = WithdrawalMethod::ofStatus(1)->exists();
        ...
        if ($hasActiveMethods) {
            $rules['withdrawal_method_id'] = 'required|exists:withdrawal_methods,id';
            $rules['withdrawal_method_fields'] = 'required|array';
        } else {
            // No configured methods: accept raw bank details from user
            $rules['bank_name'] = 'required|string|max:191';
            $rules['account_number'] = 'required|string|max:191';
        ...
        if ($user->wallet_balance < $request->amount) {
            return response()->json([
                'errors' => [
                    ['code' => 'insufficient_balance', 'message' => translate('messages.insufficient_balance')]
                ]
            ], 403);
        }
        ...
        $withdraw->withdrawal_method_id = $hasActiveMethods ? $request->withdrawal_method_id : null;

        if ($hasActiveMethods) {
            $withdraw->withdrawal_method_fields = json_encode($request->withdrawal_method_fields);
        } else {
            $withdraw->withdrawal_method_fields = json_encode([
