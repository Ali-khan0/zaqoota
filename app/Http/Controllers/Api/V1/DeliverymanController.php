...
class DeliverymanController extends Controller
{
    ...
    public function make_wallet_adjustment(Request $request)
    {
        ...
        // Credit COD parcel value to parcel senders' wallets
        $codParcelOrders = Order::where('delivery_man_id', $dm->id)
            ->where('order_type', 'parcel')
            ->where('parcel_cod', true)
            ->where('payment_method', 'cash_on_delivery')
            ->where('order_status', 'delivered')
            ->whereNotNull('user_id')
            ->where('is_guest', 0)
            ->where('parcel_value', '>', 0)
            ->whereDoesntHave('walletTransactions', function ($q) {
                $q->where('transaction_type', 'parcel_cod');
            })
            ->get();

        foreach ($codParcelOrders as $order) {
            \App\CentralLogics\CustomerLogic::create_wallet_transaction(
                $order->user_id,
                (float) $order->parcel_value,
                'parcel_cod',
                $order->id
            );
        }
    ...
    public function registration_fee_status(Request $request)
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        if (! $dm) {
        ...
        $cfg = app(DeliveryManRegistrationFeeService::class)->getSettings();
        $fee = DeliveryManRegistrationFee::where('delivery_man_id', $dm->id)->first();

        return response()->json([
            'fee_enabled' => $cfg['enabled'],
            'expected_initial_deposit' => (float) $cfg['manual_first_part'],
            'wallet_deduction_description' => $cfg['enabled'] ? translate('messages.dm_reg_fee_wallet_deduction_hint') : null,
            'registration_fee' => $fee ? [
                'total_fee' => (float) $fee->total_fee,
                'manual_paid_amount' => (float) $fee->manual_paid_amount,
                'manual_confirmed' => (bool) $fee->manual_confirmed,
                'wallet_remaining_due' => (float) $fee->wallet_remaining_due,
                'deduction_percent' => (float) $fee->deduction_percent,
                'deduction_frequency' => $fee->deduction_frequency,
                'completed' => $fee->completed_at !== null,
                'completed_at' => $fee->completed_at,
            ] : null,
        ], 200);
    }

    public function registration_fee_ledger(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'limit' => 'required',
            'offset' => 'required',
        ...
        $types = [
            DeliveryManWalletLedger::TYPE_REGISTRATION_FEE_DEDUCTION,
            DeliveryManWalletLedger::TYPE_REGISTRATION_FEE_MANUAL,
        ];

        $paginator = DeliveryManWalletLedger::where('delivery_man_id', $dm->id)
    ...
    public function milestone_bonuses(Request $request)
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        if (! $dm) {
            return response()->json(['errors' => [['code' => 'auth', 'message' => translate('messages.unauthorized')]]], 401);
        ...
        $data = app(DeliveryManMilestoneBonusService::class)->milestonesForApp($dm);

        return response()->json($data, 200);
    }
    
    public function milestone_bonus_summary(Request $request)
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        if (! $dm) {
            return response()->json(['errors' => [['code' => 'auth', 'message' => translate('messages.unauthorized')]]], 401);
        ...
        $historyMax = (int) $request->get('history_limit', 90);
        $historyMax = min(200, max(1, $historyMax));

        $payload = app(DeliveryManMilestoneBonusService::class)->buildBonusSummaryWithHistory($dm, $historyMax);
        $payload['summary']['currency'] = (string) (Helpers::currency_symbol() ?? '');
    ...
    public function bonus_transactions(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'limit' => 'required',
            'offset' => 'required',
        ...
        $totalBonus = (float) DeliveryManWalletLedger::where('delivery_man_id', $dm->id)
            ->whereIn('transaction_type', $types)
        ...
        return response()->json([
            'total_bonus_collected' => $totalBonus,
            'total_size' => $paginator->total(),
            'limit' => (int) $request['limit'],
            'offset' => (int) $request['offset'],
            'transactions' => $items,
