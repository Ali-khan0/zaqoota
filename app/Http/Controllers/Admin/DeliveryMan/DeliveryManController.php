...
class DeliveryManController extends BaseController
{
    ...
    public function getNewDeliveryManView(Request $request): View
    {
        ...
        $zone = is_numeric($zoneId) ? $this->zoneRepo->getFirstWhere(params: ['id'=>$zoneId]) : null;
        $regFeeSettings = app(DeliveryManRegistrationFeeService::class)->getSettings();

        return view(DeliveryManViewPath::NEW[VIEW], compact('deliveryMen', 'zone', 'searchBy', 'regFeeSettings'));
    }
    ...
    public function getSearchList(Request $request): JsonResponse
    {
        $deliveryMen = $this->deliveryManRepo->getListWhere(
            searchValue: $request['search'],
            filters: ['type' => 'zone_wise','application_status' => 'approved'],
            relations: ['zone', 'registrationFee'],
        );
    ...
    {
        $dm = $this->deliveryManRepo->add(data: $this->deliveryManService->getAddData(request: $request));
    ...
    public function getUpdateView(string|int $id): View
    {
        ...
        $language = getWebConfig('language');
        $defaultLang = str_replace('_', '-', app()->getLocale());
        $registrationFee = DeliveryManRegistrationFee::where('delivery_man_id', $id)->first();
        $regFeeSettings = app(DeliveryManRegistrationFeeService::class)->getSettings();

        return view(DeliveryManViewPath::UPDATE[VIEW], compact('deliveryMan', 'language', 'defaultLang', 'registrationFee', 'regFeeSettings'));
    }
    ...
    public function getPreview(Request $request, int|string $id, string $tab='info'): View
    {
        ...
        if($tab == 'info')
        {
            ...
            return view(DeliveryManViewPath::INFO[VIEW], compact('deliveryMan', 'reviews', 'regFeeSettings'));
        }
    ...
    public function approveDeliveryManApplication(Request $request): RedirectResponse
    {
        $request->validate([
            'id' => 'required|integer|exists:delivery_men,id',
            'initial_deposit_received' => 'nullable|boolean',
            'initial_deposit_amount' => 'nullable|numeric|min:0',
        ]);

        $feeService = app(DeliveryManRegistrationFeeService::class);
        $cfg = $feeService->getSettings();

        $deliveryMan = $this->deliveryManRepo->getFirstWhere(params: ['id' => $request->id, 'type' => 'zone_wise']);
        if (! $deliveryMan) {
        ...
        if ($deliveryMan->application_status === 'approved') {
            Toastr::warning(translate('messages.application_status_updated_successfully'));

            return back();
        }

        if ($cfg['enabled'] && $cfg['require_initial_on_approve'] && ! $request->boolean('initial_deposit_received')) {
            Toastr::error(translate('messages.dm_approve_confirm_required'));
        ...
        if ($cfg['enabled'] && $request->boolean('initial_deposit_received')) {
            $raw = $request->input('initial_deposit_amount');
            $amount = ($raw !== null && $raw !== '' && (float) $raw > 0)
                ? (float) $raw
                : $cfg['manual_first_part'];
            $feeService->confirmManualPayment((int) $request->id, $amount);
        }
    ...
    public function updateApplication(Request $request): RedirectResponse
    {
        if (($request['status'] ?? '') === 'approved') {
            ...
            return $this->approveDeliveryManApplication($forward);
        }
