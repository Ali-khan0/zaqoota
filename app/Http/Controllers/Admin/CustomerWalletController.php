...
use App\Exports\CustomerWithdrawTransactionExport;
...
class CustomerWalletController extends Controller
{
    public function withdraw(Request $request)
    {
        ...
        $withdraw_req = \App\Models\WithdrawRequest::with(['user', 'method'])
            ...
            ->when(count(array_filter($key)) > 0, function ($query) use ($key) {
        ...
        return view('admin-views.wallet.customer-withdraw', compact('withdraw_req'));
    }
    ...
    {
        $withdraw = WithdrawRequest::with(['user', 'method'])->where(['id' => $request->withdraw_id])->first();
        if (!$withdraw || !$withdraw->user) {
            return response()->json([
                'view' => '<div class="text-center py-4">'.translate('messages.no_data_found').'</div>',
            ]);
        }

        return response()->json([
            'view' => view('admin-views.wallet.customer-partials._side_view', compact('withdraw'))->render(),
        ]);
    }
    ...
    public function withdraw_export(Request $request)
    {
        ...
        $withdraw_req = WithdrawRequest::with(['user'])
            ->when($all, function ($query) {
                return $query;
            })
            ->when($active, function ($query) {
            ...
            ->when(isset($key) && count(array_filter($key)) > 0, function ($query) use ($key) {
                return $query->whereHas('user', function ($query) use ($key) {
                    foreach ($key as $value) {
                        if ($value === '' || $value === null) {
                            continue;
        ...
        if ($request->type == 'excel') {
            return Excel::download(new CustomerWithdrawTransactionExport($data), 'CustomerWithdrawRequests.xlsx');
        }
        if ($request->type == 'csv') {
            return Excel::download(new CustomerWithdrawTransactionExport($data), 'CustomerWithdrawRequests.csv');
        }
    }
    ...
    public function withdrawStatus(Request $request, $id)
    {
        ...
        if ((int)$withdraw->approved !== 0) {
            Toastr::info('Withdraw request is already processed.');
            return back();
        }
        ...
        $withdraw->approved = (int)$request->approved;
        $withdraw->transaction_note = $request->note;

        if ((int)$request->approved === 1) {
            if ((float)$user->wallet_balance < (float)$withdraw->amount) {
                Toastr::error(translate('messages.insufficient_balance'));
                return back();
            }

            $user->decrement('wallet_balance', $withdraw->amount);
            $withdraw->save();

            Toastr::success('Customer withdraw request approved.');
        } else {
            $withdraw->save();
            Toastr::info('Customer withdraw request denied.');
        }

        return redirect()->route('admin.transactions.customer.withdraw_list');
    }
