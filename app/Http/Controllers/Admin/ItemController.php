...
class ItemController extends Controller
{
    ...
    public function all_products_with_stores(Request $request)
    {
        $store_id = $request->query('store_id', 'all');
        $status_filter = $request->query('status', 'all'); // all, pending, approved
        $key = explode(' ', $request['search'] ?? '');
        
        // Get pending items from TempProduct
        $pending_items = TempProduct::withoutGlobalScope(StoreScope::class)
            ->with(['store', 'category'])
            ->when(is_numeric($store_id), function ($query) use ($store_id) {
                return $query->where('store_id', $store_id);
            })
            ...
            ->map(function ($item) {
                return [
                    ...
                    'store_name' => $item->store?->name ?? translate('messages.store_deleted'),
                    'category_name' => $item->category?->name ?? translate('messages.category_deleted'),
                    'status' => $item->is_rejected == 1 ? 'rejected' : 'pending',
                    'type' => 'temp_product',
                    'created_at' => $item->created_at,
                ];
            });

        // Get approved items from Item
        $approved_items = Item::withoutGlobalScope(StoreScope::class)
            ->with(['store', 'category'])
            ->when(is_numeric($store_id), function ($query) use ($store_id) {
                return $query->where('store_id', $store_id);
            })
        ...
        $all_items = $pending_items->concat($approved_items);
        
        if ($status_filter == 'pending') {
            $all_items = $all_items->where('status', 'pending');
        } elseif ($status_filter == 'approved') {
            $all_items = $all_items->where('status', 'approved');
        }

        // Sort by created_at desc and paginate
        $all_items = $all_items->sortByDesc('created_at');
        $per_page = config('default_pagination');
        $current_page = $request->get('page', 1);
        $items = new \Illuminate\Pagination\LengthAwarePaginator(
            $all_items->forPage($current_page, $per_page),
            $all_items->count(),
            $per_page,
            $current_page,
        ...
        return view('admin-views.product.all-products-stores', compact('items', 'store', 'status_filter'));
    }
