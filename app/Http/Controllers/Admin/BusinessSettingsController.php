...
class BusinessSettingsController extends Controller
{
    ...
    public function mail_config(Request $request)
    {
        ...
        try {
            ...
            if (file_exists($envPath)) {
                ...
                $updateEnvVar('MAIL_MAILER', $request['driver'] ?? 'smtp');
                $updateEnvVar('MAIL_HOST', $request['host'] ?? '');
                $updateEnvVar('MAIL_PORT', $request['port'] ?? '587');
                $updateEnvVar('MAIL_USERNAME', $request['username'] ?? '');
                $updateEnvVar('MAIL_PASSWORD', $request['password'] ?? '');
                $updateEnvVar('MAIL_ENCRYPTION', $request['encryption'] ?? 'tls');
                $updateEnvVar('MAIL_FROM_ADDRESS', $request['email'] ?? '');
                $updateEnvVar('MAIL_FROM_NAME', $request['name'] ?? '');
                
                file_put_contents($envPath, $envContent);
            }
        } catch (\Exception $ex) {
            info('Failed to update .env file: ' . $ex->getMessage());
            // Continue even if .env update fails - database config will still work
        }
    ...
    {
        $this->ensureAssanPayGatewaySettingExists();
        $this->mergeAssanPayLegacyCredentialKeys();
        ...
        $data_values = Setting::whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'assan_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago', 'payfast'])->get();

        return view('admin-views.business-settings.payment-index', compact('published_status', 'payment_url', 'data_values'));
    }
    
    /**
     * AssanPay row in addon_settings (matches merchant dashboard: API key + decryption key; see payment-index hints for URLs).
     */
    private function ensureAssanPayGatewaySettingExists(): void
    {
        if (!Schema::hasTable('addon_settings')) {
            return;
        }

        if (Setting::where('key_name', 'assan_pay')->where('settings_type', 'payment_config')->exists()) {
            return;
        }
    ...
    /**
     * Copy merchant_id/secret_key into api_key/decryption_key for stores that saved before field rename.
     */
    private function mergeAssanPayLegacyCredentialKeys(): void
    {
        $row = Setting::where('key_name', 'assan_pay')->where('settings_type', 'payment_config')->first();
        if (!$row) {
            return;
        }

        $live = $row->live_values;
        $test = $row->test_values;
        if (!is_array($live)) {
            return;
        }

        $patch = function (array $values): array {
            if (!array_key_exists('merchant_id', $values)) {
                $values['merchant_id'] = '';
            }
            if (!array_key_exists('api_key', $values)) {
                $values['api_key'] = '';
            }
            if (!array_key_exists('decryption_key', $values)) {
                $values['decryption_key'] = '';
            }
            if (empty($values['decryption_key']) && !empty($values['secret_key'])) {
                $values['decryption_key'] = $values['secret_key'];
            }
            if (empty($values['checkout_url'])) {
                $values['checkout_url'] = 'https://api.assanpay.com';
            }
            if (!array_key_exists('callback_url', $values)) {
                $values['callback_url'] = '';
            }
        ...
        $newLive = $patch($live);
        $newTest = is_array($test) ? $patch($test) : $newLive;

        if ($newLive !== $live || $newTest !== $test) {
            $row->live_values = $newLive;
            $row->test_values = $newTest;
            $row->save();
        }
    }
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        if ($name == 'cash_on_delivery') {
            ...
            Helpers::businessUpdateOrInsert(['key' => 'payfast'], [
                'value' => json_encode([
                    'status' => $request['status'],
                    'merchant_id' => $request['merchant_id'] ?? '',
    ...
    public function payment_config_update(Request $request)
    {
        ...
        $validation = [
            'gateway' => 'required|in:ssl_commerz,paypal,stripe,razor_pay,senang_pay,assan_pay,paytabs,paystack,paymob_accept,paytm,flutterwave,liqpay,bkash,mercadopago,payfast',
        ...
        } elseif ($request['gateway'] == 'assan_pay') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'merchant_id' => 'required_if:status,1',
                'api_key' => 'required_if:status,1',
                'decryption_key' => 'nullable|string|max:2048',
                'checkout_url' => 'nullable|string|max:2048',
    ...
    public function services_policy()
    {
        $services_policy = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'services_policy')->first();

        return view('admin-views.business-settings.services-policy', compact('services_policy'));
    }

    public function services_policy_update(Request $request)
    {
        $this->update_data($request, 'services_policy');
        Toastr::success(translate('messages.services_policy_updated'));
