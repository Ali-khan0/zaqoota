...
class HomeController extends Controller
{
    ...
    public function account_deletion_request()
    {
        ...
        if (isset($config) && $config) {
            return view('account-deletion-request', compact('custome_recaptcha'));
        } elseif ($landing_integration_type == 'file_upload' && File::exists('resources/views/layouts/landing/custom/index.blade.php')) {
            return view('layouts.landing.custom.index');
        } elseif ($landing_integration_type == 'url') {
            return redirect($redirect_url);
    ...
    public function submit_account_deletion_request(Request $request)
    {
        $request->validate([
        ...
        if ($existingRequest) {
            Toastr::warning(translate('You already have a pending account deletion request. Please wait for admin approval.'));
            return back();
        }

        $deletionRequest = new AccountDeletionRequest;
        $deletionRequest->email = $request->email;
        $deletionRequest->reason = $request->reason;
        $deletionRequest->user_id = $user ? $user->id : null;
        $deletionRequest->status = 'pending';
        $deletionRequest->save();

        Toastr::success(translate('Account deletion request submitted successfully! Admin will review your request.'));
        return back();
    }
    ...
    public function services_policy(Request $request)
    {
        $data = self::get_settings('services_policy');
        ...
        if (isset($config) && $config) {
            return view('services-policy', compact('data'));
        } elseif ($landing_integration_type == 'file_upload' && File::exists('resources/views/layouts/landing/custom/index.blade.php')) {
            return view('layouts.landing.custom.index');
        } elseif ($landing_integration_type == 'url') {
            return redirect($redirect_url);
