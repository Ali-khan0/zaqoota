...
use App\Http\Controllers\AssanPayController;
...
Route::get('account-deletion-request', 'HomeController@account_deletion_request')->name('account-deletion-request');
Route::post('submit-account-deletion-request', 'HomeController@submit_account_deletion_request')->name('submit-account-deletion-request');
Route::get('services-policy', 'HomeController@services_policy')->name('services-policy');
...
if (!$is_published) {
    Route::group(['prefix' => 'payment'], function () {
      ...
      Route::group(['prefix' => 'razor-pay', 'as' => 'razor-pay.'], function () {
        ...
        //ASSAN-PAY
        Route::group(['prefix' => 'assan-pay', 'as' => 'assan-pay.'], function () {
            Route::get('pay', [AssanPayController::class, 'index'])->name('pay');
            Route::any('callback', [AssanPayController::class, 'callback'])
                ->name('callback')
                ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });
...
// Smart QR Code App Router
Route::get('/download-app', function (\Illuminate\Http\Request $request) {
    // Get the browser's agent configuration
    $userAgent = strtolower($request->header('User-Agent'));

    // --- CONFIGURATION ---
    // Replace these placeholder links with your real application locations:
    $playStoreUrl = 'https://play.google.com/store/apps/details?id=com.freelince.zaqoota_mart_user';
    $webAppUrl    = 'https://app.zaqoota.com'; 
    // ---------------------

    // Detect if scanning device runs Android OS
    if (str_contains($userAgent, 'android')) {
        return redirect()->away($playStoreUrl);
    }

    // Default Fallback: Instantly pushes iOS (iPhones/iPads) and Desktop/Mac to the Web App
